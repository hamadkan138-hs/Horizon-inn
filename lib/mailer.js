  ...
  <div style="font-family: Georgia, 'Times New Roman', serif; max-width:600px; margin:0 auto; color:#23242c;">
    ...
    <table cellpadding="0" cellspacing="0" style="margin: 18px 0;">
      <tr>
        ...
        ${statTile('Total Revenue', money(summary.totalReceivedToday))}
      ...
      <tr>
        ...
        ${statTile('Bank Transfers', money(summary.bankReceivedToday))}
        ${statTile('Online Payments', money(summary.onlineReceivedToday))}
      </tr>
      <tr><td colspan="10" style="height:10px;"></td></tr>
      <tr>
        ${statTile('Expenses Today', money(summary.expensesTotalToday))}
        ${statTile('Net Cash Today', money(summary.netCashToday))}
      </tr>
    </table>
    ...
    ${summary.expensesToday.length ? `
      <h3 style="color:#14161f; font-size:15px; margin-bottom:6px;">Today's Expenses</h3>
      <table cellpadding="0" cellspacing="0" style="width:100%; border-collapse:collapse; font-size:14px; margin-bottom:20px;">
        ${summary.expensesToday.map((e) => `
          <tr>
            <td style="padding:8px 10px; border-bottom:1px solid #ece8de;">${escapeHtml(e.category)}</td>
            <td style="padding:8px 10px; border-bottom:1px solid #ece8de;">${escapeHtml(e.description || '')}</td>
            <td style="padding:8px 10px; border-bottom:1px solid #ece8de; text-align:right;">${money(e.amount)}</td>
          </tr>
        `).join('')}
      </table>
    ` : ''}
...
function renderSummaryText(summary) {
  const lines = [
    ...
    `Total revenue today: ${money(summary.totalReceivedToday)}`,
    `  Cash: ${money(summary.cashReceivedToday)}`,
    `  Bank transfers: ${money(summary.bankReceivedToday)}`,
    `  Online payments: ${money(summary.onlineReceivedToday)}`,
    `Expenses today: ${money(summary.expensesTotalToday)}`,
    `Net cash today: ${money(summary.netCashToday)}`,
    ...
    'Today\'s expenses:',
    ...(summary.expensesToday.length
      ? summary.expensesToday.map((e) => `  - ${e.category}${e.description ? ` (${e.description})` : ''} — ${money(e.amount)}`)
      : ['  (none)'])
  ];
  return lines.join('\n');
}
...
function invoiceLink(booking) {
  return `${SITE_URL}/invoice.html?id=${booking.id}&token=${booking.invoice_token}`;
}

function renderInvoiceEmailHtml(booking, room) {
  const link = invoiceLink(booking);
  return `
  <div style="font-family: Georgia, 'Times New Roman', serif; max-width:600px; margin:0 auto; color:#23242c;">
    <h2 style="color:#14161f; border-bottom:2px solid #c6a15b; padding-bottom:10px; margin-bottom:4px;">Your Invoice &mdash; Horizon Inn</h2>
    <p>Dear ${escapeHtml(booking.name)},</p>
    <p>Here is your invoice for ${escapeHtml(room.name)}, ${escapeHtml(booking.checkin)} to ${escapeHtml(booking.checkout)}.</p>

    <table cellpadding="0" cellspacing="0" style="width:100%; border-collapse:collapse; font-size:14px; margin:16px 0;">
      <tr><td style="padding:6px 0; color:#6f7180;">Invoice Number</td><td style="padding:6px 0; text-align:right; font-weight:700;">${escapeHtml(booking.invoice_number)}</td></tr>
      <tr><td style="padding:6px 0; color:#6f7180;">Total Amount</td><td style="padding:6px 0; text-align:right; font-weight:700;">${money(booking.total_amount)}</td></tr>
      <tr><td style="padding:6px 0; color:#6f7180;">Payment Status</td><td style="padding:6px 0; text-align:right; text-transform:capitalize;">${escapeHtml(booking.payment_status)}</td></tr>
    </table>

    <p style="text-align:center; margin:26px 0;">
      <a href="${link}" style="background:#14161f; color:#ffffff; text-decoration:none; padding:12px 28px; font-size:13px; letter-spacing:0.08em; text-transform:uppercase;">View / Print Invoice</a>
    </p>

    <p style="color:#6f7180; font-size:12px; margin-top:28px; border-top:1px solid #ece8de; padding-top:12px;">
      Thank you for staying with Horizon Inn.
    </p>
  </div>
  `;
}

function renderInvoiceEmailText(booking, room) {
  return [
    `Your Invoice — Horizon Inn`,
    '',
    `Dear ${booking.name},`,
    `Here is your invoice for ${room.name}, ${booking.checkin} to ${booking.checkout}.`,
    '',
    `Invoice Number: ${booking.invoice_number}`,
    `Total Amount: ${money(booking.total_amount)}`,
    `Payment Status: ${booking.payment_status}`,
    '',
    `View / print your invoice: ${invoiceLink(booking)}`
  ].join('\n');
}

async function sendInvoiceEmail(booking, room) {
  const transporter = getTransporter();
  if (!transporter) {
    console.warn('[invoiceEmail] Skipped — EMAIL_USER / EMAIL_APP_PASSWORD are not configured.');
    return { sent: false, reason: 'Email is not configured on this server.' };
  }
  if (!booking.email) return { sent: false, reason: 'No guest email on file for this booking' };

  await transporter.sendMail({
    from: `"Horizon Inn" <${process.env.EMAIL_USER}>`,
    to: booking.email,
    subject: `Your Horizon Inn Invoice — ${booking.invoice_number}`,
    html: renderInvoiceEmailHtml(booking, room),
    text: renderInvoiceEmailText(booking, room)
  });
...
module.exports = { sendDailySummaryEmail, sendBookingConfirmationEmail, sendInvoiceEmail, invoiceLink, SITE_URL, getTransporter };
